    ...
    <div className="w-full">
      <header className="mb-6 flex items-center justify-end gap-5">
        <div className="flex items-center gap-4">
          <BrandMark />
          <button
            className="min-h-11 rounded-xl border border-gray-300 bg-white px-4 font-bold text-black transition-colors hover:border-green-500 dark:border-[#2b2b2b] dark:bg-[#0b0b0b] dark:text-white dark:hover:border-[#deff9a]/55"
            onClick={onLogout}
            type="button"
          >
            Logout
      ...
      <main className="grid min-w-0 gap-5 rounded-[34px] bg-gray-50 p-5 shadow-sm transition-colors dark:bg-[radial-gradient(circle_at_45%_40%,rgba(218,255,154,0.08),transparent_32rem),linear-gradient(145deg,rgba(26,30,26,0.96),rgba(5,8,5,0.98))] dark:shadow-[0_24px_80px_rgba(0,0,0,0.6)] sm:p-7">
        <section className="grid min-w-0 gap-5 lg:grid-cols-3">
          ...
          <SummaryCard accent="text-green-600 dark:text-[#deff9a]" amount={monthlyIncome} title="Monthly Income" currency={userCurrency} salaryShield={salaryShield} hourlyWage={hourlyWage} />
          <SummaryCard accent="text-red-500 dark:text-red-300" amount={monthlyExpenses} title="Monthly Expenses" currency={userCurrency} salaryShield={salaryShield} hourlyWage={hourlyWage} />
        </section>
